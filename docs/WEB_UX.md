...
- **Tablet and phone-width browser** (approved and implemented): one consolidated adaptation of that same desktop interface — not a separate mobile product, and not a redesign. Desktop remains the visual source of truth: almost every rule sits inside a `max-width` media query, the one deliberate exception being the shared compact-header geometry below, which also corrects a hamburger/title overlap at desktop widths.
...
**Compact headers.** Compact pages read as two columns: a small hamburger column at the far left, then the page column holding everything else. The shell's top bar overlays the header band, so the hamburger stays on the *same row* as the page title and the account control stays on that row too — the hamburger is never stacked above the title. The column's width is one shared value, `--compact-hamburger-column` (the control's real 40px width plus a 16px gap), and it insets the page *content*, not just the title: title, subtitle, cards, filters and tables all begin where that column ends, on one alignment line, while the hamburger sits outside it to the left. The page action and the account control sit at the right of that same top row as one compact action group: the account control finishes on the page content's own right boundary, and a single shared reserve-plus-gap pair holds the page action just to its left, so the two stay grouped rather than spreading across the header width. The same pair applies wherever the bar overlays the header, so no page sets its own. All five compact-header pages — Dashboard, Returns, Return Details, Users, Routes — use this shared geometry and none sets a value of its own. At drawer widths the bar leaves the overlay and wraps to its own row, releasing the column so page content spans the full viewport minus its gutter.
