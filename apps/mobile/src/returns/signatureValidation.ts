...
/** Mirrors `returnrecord.ReturnSignatureService`'s limits exactly — the backend remains the final source of truth. */
export const SIGNER_NAME_MAX_LENGTH = 100;

/**
 * Same normalized (0..1) minimum-total-drawn-length threshold as the
 * backend's `ReturnSignatureService.MIN_TOTAL_PATH_LENGTH_NORMALIZED` —
 * kept in sync manually since there is no shared contract package. Used
 * only for immediate client-side feedback; the backend independently
...
export function validateSignerName(rawSignerName: string): string | undefined {
  ...
  if (trimmed.length > SIGNER_NAME_MAX_LENGTH) {
    return `Signer name must be ${SIGNER_NAME_MAX_LENGTH} characters or fewer.`;
  }
  return undefined;
}
...
/** True only once the driver has drawn enough to plausibly be a real signature, not just a tap or a barely-moved touch. */
