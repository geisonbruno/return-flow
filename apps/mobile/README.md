...
The `local` profile allows exactly the browser origin `http://localhost:8081` to call the API (`app.cors.allowed-origins` in `application-local.properties`; see `auth.security.CorsProperties`) — a narrow, explicit allowance for local development. The same property is how a deployed environment names its own Web origin, and it is empty by default, so no browser origin is allowed anywhere unless it is configured. Native iOS/Android requests never go through a browser and never need CORS at all.
